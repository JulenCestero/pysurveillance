...
def first_grade_analysis(
    df: pd.DataFrame, authors: pd.DataFrame, affiliations: pd.DataFrame
    ...
    publications_per_author = pd.DataFrame(
        data={author: [len(list(filter(lambda x: author in x, df["Authors"])))] for author in authors}
    ).T
    publications_per_author.columns = ["Publications"]

    publications_per_affiliations = pd.DataFrame(
        data={
            affiliation: [len(list(filter(lambda x: affiliation == x, df["Affiliations"])))]
...
def second_grade_analysis(
    df: pd.DataFrame, authors: pd.DataFrame, sources: pd.DataFrame, papers: pd.DataFrame
) -> Union[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    Processing data
    """
    cites_per_author = pd.DataFrame(
        data={author: [df[df["Authors"].str.contains(author, na=False)]["Cited by"].sum()] for author in authors}
    ).T
    cites_per_author.columns = ["Cites"]
    cites_per_source = pd.DataFrame(
        data={
            source: [df[df["Source title"].str.contains(source, na=False)]["Cited by"].sum()]
...
def plot_fourth_grade_analysis(scimago: pd.DataFrame, scimago_years: list, scimago_avaliable: bool) -> None:
    if scimago is not None:
        ...
        quartiles = []
        for source, paper, cites, year in zip(
            list(top_papers.Source), list(top_papers.Cites.index), list(top_papers.Cites), list(top_papers.Year)
        ):
        ...
        c = (
            ...
            .mark_circle()
            .encode(
                x="papers", y="cites", size="cites", color="year", tooltip=["papers", "source", "cites", "year", "quartiles"]
            )
        )
